...
import { SetBusinessParams, Result, SuccessResponse, ErrorResponse, DeleteBusinessParams } from '../shared/types';
import { Business } from '@sirge-io/sirge-utils';
import { buildTestEvent } from './helpers/buildTestEvent';
import { getTestContext } from './helpers/getTestContext';
import { handler } from '../lambda-handlers/delete-business-by-businessid';

require('dotenv').config();

describe('Test for app handler', () => {
  let context: Context;
  let callback: Callback<Result<boolean | null>>;
  ...
  it('verifies successful response', async () => {
    const event = buildTestEvent<DeleteBusinessParams>({
