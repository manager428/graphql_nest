...
import { getVisitorDetailPageView } from '../shared/postgresDb';
...
export type GetVisitorDetailPageViewParams = {
  getVisitorDetailPageViewInput: {
    business_id: string;
    visitor_id: string;
    page: number;
  };
...
export const handler: AppSyncResolverHandler<
  GetVisitorDetailPageViewParams,
  Result<PageView[]>
> = Sentry.AWSLambda.wrapHandler(async (event, _context) => {
  try {
    ...
    const {
      getVisitorDetailPageViewInput: { business_id, visitor_id, page },
    } = event.arguments as GetVisitorDetailPageViewParams;
    ...
    const visitor_pageviews = await getVisitorDetailPageView(
...
if (require.main == module) {
  enableDebugMode<GetVisitorDetailPageViewParams, Result<PageView[]>>(
    handler,
    buildTestEvent<GetVisitorDetailPageViewParams>({
