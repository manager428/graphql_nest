...
import { verifyUserSubscription, checkBusiness } from '../shared/mongoDb';
...
> = Sentry.AWSLambda.wrapHandler(async (event, _context) => {
  try {
    ...
    const response = await getVisitorsOfBusiness(
    ...
    logInfo(`Business Visitor returned: ${JSON.stringify(response)}`);

    return successResponse(response, 'Business Visitor returned');
  } catch (error) {
